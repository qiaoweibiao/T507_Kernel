...
static const struct snd_soc_dapm_widget sunxi_codec_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MIXER("ADCR Input", SND_SOC_NOPM, 0, 0,
	...
	SND_SOC_DAPM_SPK("HpSpeaker", sunxi_codec_speaker_event),
};
			...
			int clk_id, unsigned int freq, int dir)
{
	...
	if (dai->playback_active) {
		if (clk_set_rate(sunxi_codec->pllclkx4, freq * 4)) {
			pr_err("audiocodec: codec source set pllclkx4 rate failed\n");
			return -EINVAL;
		}

		/* moduleclk freq = 24.576/22.5792M, audio clk source = 98.304/90.3168M */
...
static int sunxi_internal_codec_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(sunxi_codec->vol_supply.avcc)) {
		...
		sunxi_codec->vol_supply.avcc = NULL;
	} else {
		ret = regulator_enable(sunxi_codec->vol_supply.avcc);
		if (ret) {
			pr_err("[%s]:avcc enable failed!\n", __func__);
