 ...
 *				Efuse related code
 * *********************************************************** */
static u8 hal_EfuseSwitchToBank(PADAPTER padapter, u8 bank, u8 bPseudoTest)
{
	u8 bRet = _FALSE;
	u32 value32 = 0;
...
void rtw_efuse_analyze(PADAPTER padapter, u8 Type, u8 Fake)
{
	...
	u8 efuseHeader = 0, efuseExtHdr = 0,
	   efuseData[EFUSE_MAX_WORD_UNIT * 2] = {0}, dataCnt = 0;
	...
	eFuseWord =
	    rtw_zmalloc(EFUSE_MAX_SECTION_NUM * (EFUSE_MAX_WORD_UNIT * 2));
	...
	RTW_INFO("\n\r 2Byte header, extHeader[7:4]=offset[6:3], "
		 "extHeader[3:0]=word enable\n");

	EFUSE_GetEfuseDefinition(padapter, efuseType, TYPE_EFUSE_MAP_LEN,
				 (PVOID)&mapLen, bPseudoTest);
	EFUSE_GetEfuseDefinition(padapter, efuseType, TYPE_EFUSE_MAX_SECTION,
				 (PVOID)&max_section, bPseudoTest);
	EFUSE_GetEfuseDefinition(padapter, efuseType,
				 TYPE_EFUSE_PROTECT_BYTES_BANK,
				 (PVOID)&protectBytesBank, bPseudoTest);
	EFUSE_GetEfuseDefinition(padapter, efuseType,
				 TYPE_EFUSE_CONTENT_LEN_BANK, (PVOID)&efuse_max,
				 bPseudoTest);
	EFUSE_GetEfuseDefinition(padapter, EFUSE_WIFI,
				 TYPE_EFUSE_REAL_CONTENT_LEN, (PVOID)&maprawlen,
				 _FALSE);

	_rtw_memset(eFuseWord, 0xff,
		    EFUSE_MAX_SECTION_NUM * (EFUSE_MAX_WORD_UNIT * 2));
	...
	for (bank = startBank; bank <= endBank; bank++) {
		...
		while ((efuseHeader != 0xFF) &&
		       ((efuseType == EFUSE_WIFI && (eFuse_Addr < maprawlen)) ||
			(efuseType == EFUSE_BT &&
			...
			if (offset < max_section) {
				for (i = 0; i < EFUSE_MAX_WORD_UNIT; i++) {
					/* Check word enable condition in the
					 * section	*/
					if (!(wden & (0x01 << i))) {
						if (!((efuseType ==
							   EFUSE_WIFI &&
			...
			if (bExtHeader) {
				ParseEfuseExtHdr =
				    (efuseHeader2Byte & 0xff00) >> 8;
				ParseEfuseHeader = (efuseHeader2Byte & 0xff);
				ParseOffset2_0 =
				    GET_HDR_OFFSET_2_0(ParseEfuseHeader);
				ParseOffset = ((ParseEfuseExtHdr & 0xF0) >> 1) |
					      ParseOffset2_0;
				ParseWDEN = (ParseEfuseExtHdr & 0x0F);
				RTW_INFO("Header=0x%x, ExtHeader=0x%x, ",
					 ParseEfuseHeader, ParseEfuseExtHdr);
			} else {
				...
				if (!(wden & (0x01 << i))) {
					RTW_INFO("Map[ %02X ] = %02X %02X\n",
						 ((offset *
						  ...
						  (i * 2)),
						 efuseData[wd_cnt * 2 + 0],
						 efuseData[wd_cnt * 2 + 1]);
					wd_cnt++;
				}
			}
			...
			efuse_OneByteRead(padapter, eFuse_Addr++, &efuseHeader,
					  bPseudoTest);
			if (efuseHeader == 0xFF) {
				if ((eFuse_Addr + protectBytesBank) >=
				    efuse_max)
					bCheckNextBank = TRUE;
				else
		...
		if (!bCheckNextBank) {
			RTW_INFO("Not need to check next bank, eFuse_Addr=%d, "
				 "protectBytesBank=%d, efuse_max=%d\n",
	...
	for (i = 0; i < max_section; i++) {
		for (j = 0; j < EFUSE_MAX_WORD_UNIT; j++) {
			pEfuseHal->fakeEfuseInitMap[(i * 8) + (j * 2)] =
			    (eFuseWord[(i * 8) + (j * 2)]);
			pEfuseHal->fakeEfuseInitMap[(i * 8) + ((j * 2) + 1)] =
			    (eFuseWord[(i * 8) + ((j * 2) + 1)]);
		}
	}
...
out_free_buffer:
	if (eFuseWord)
		rtw_mfree((u8 *)eFuseWord,
			  EFUSE_MAX_SECTION_NUM * (EFUSE_MAX_WORD_UNIT * 2));
}

VOID efuse_PreUpdateAction(PADAPTER pAdapter, pu4Byte BackupRegs)
{
	if (IS_HARDWARE_TYPE_8812AU(pAdapter) ||
	    IS_HARDWARE_TYPE_8822BU(pAdapter)) {
		/* <20131115, Kordan> Turn off Rx to prevent from being busy
		 * when writing the EFUSE. (Asked by Chunchu.)*/
		BackupRegs[0] =
		    phy_query_mac_reg(pAdapter, REG_RCR, bMaskDWord);
		BackupRegs[1] =
		    phy_query_mac_reg(pAdapter, REG_RXFLTMAP0, bMaskDWord);
		BackupRegs[2] =
		    phy_query_mac_reg(pAdapter, REG_RXFLTMAP0 + 4, bMaskDWord);
#ifdef CONFIG_RTL8812A
		BackupRegs[3] =
		    phy_query_mac_reg(pAdapter, REG_AFE_MISC, bMaskDWord);
#endif
...
#ifdef CONFIG_RTL8812A
		/* <20140410, Kordan> 0x11 = 0x4E, lower down LX_SPS0 voltage.
		 * (Asked by Chunchu)*/
...
VOID efuse_PostUpdateAction(PADAPTER pAdapter, pu4Byte BackupRegs)
{
	if (IS_HARDWARE_TYPE_8812AU(pAdapter) ||
	    IS_HARDWARE_TYPE_8822BU(pAdapter)) {
		/* <20131115, Kordan> Turn on Rx and restore the registers.
		 * (Asked by Chunchu.)*/
		phy_set_mac_reg(pAdapter, REG_RCR, bMaskDWord, BackupRegs[0]);
		phy_set_mac_reg(pAdapter, REG_RXFLTMAP0, bMaskDWord,
				BackupRegs[1]);
		phy_set_mac_reg(pAdapter, REG_RXFLTMAP0 + 4, bMaskDWord,
				BackupRegs[2]);
#ifdef CONFIG_RTL8812A
		phy_set_mac_reg(pAdapter, REG_AFE_MISC, bMaskDWord,
				BackupRegs[3]);
#endif
		RTW_INFO(" %s , done\n", __func__);
	}
...
void EFUSE_GetEfuseDefinition(PADAPTER adapter, u8 efusetype, u8 type,
			      void *out, BOOLEAN test)
{
	struct dvobj_priv *d;
	u32 v32 = 0;
	...
	if (adapter->hal_func.EFUSEGetEfuseDefinition) {
		adapter->hal_func.EFUSEGetEfuseDefinition(adapter, efusetype,
							  type, out, test);
		return;
	}
...
u8 rtw_efuse_access(PADAPTER adapter, u8 write, u16 addr, u16 cnts, u8 *data)
{
	...
	if (_TRUE == write) {
		...
		if (efuse) {
			err =
			    rtw_halmac_read_physical_efuse_map(d, efuse, size);
			if (err) {
			...
			err =
			    rtw_halmac_read_physical_efuse(d, addr, cnts, data);
			if (err)
				return _FAIL;
		}
	}
...
u8 rtw_efuse_bt_access(PADAPTER adapter, u8 write, u16 addr, u16 cnts, u8 *data)
{
	...
	if (_TRUE == write) {
		...
		if (err == -1) {
			RTW_ERR(
			    "%s: rtw_halmac_write_bt_physical_efuse fail!\n",
			    __FUNCTION__);
			return _FAIL;
		}
		RTW_INFO(
		    "%s: rtw_halmac_write_bt_physical_efuse OK! data 0x%x\n",
		    __FUNCTION__, *data);
	} else {
		efuse = rtw_zmalloc(size);

		if (efuse) {
			err = rtw_halmac_read_bt_physical_efuse_map(d, efuse,
								    size);

			if (err == -1) {
				RTW_ERR("%s: "
					"rtw_halmac_read_bt_physical_efuse_map "
					"fail!\n",
					__FUNCTION__);
				rtw_mfree(efuse, size);
				return _FAIL;
			...
			RTW_INFO("%s: rtw_halmac_read_bt_physical_efuse_map "
				 "ok! data 0x%x\n",
				 __FUNCTION__, *data);
			rtw_mfree(efuse, size);
		}
...
u8 rtw_efuse_map_read(PADAPTER adapter, u16 addr, u16 cnts, u8 *data)
{
	...
	if (efuse) {
		err =
		    rtw_halmac_read_logical_efuse_map(d, efuse, size, NULL, 0);
		if (err) {
			rtw_mfree(efuse, size);
			status = _FAIL;
			goto exit;
	...
	if (adapter->registrypriv.boffefusemask == 0) {
		...
		if (mask_len != 0) {
			...
			err = rtw_halmac_write_logical_efuse_map(
			    d, efuse, size, mask_buf, mask_len);
		} else
			err = rtw_halmac_write_logical_efuse_map(d, efuse, size,
								 NULL, 0);
	} else {
...
int Efuse_PgPacketWrite(PADAPTER adapter, u8 offset, u8 word_en, u8 *data,
			BOOLEAN test)
{
	return _FALSE;
}
...
#define RT_ASSERT_RET(expr)                                                    \
	...
	RT_ASSERT_RET((mapLen & 0x7) ==
		      0); /* have to be PGPKT_DATA_SIZE alignment for memcpy */
	...
	for (i = 0; i < mapLen;
	     i += 16) { /* set 512 because the iwpriv's extra size have limit
			   0x7FF */
		RTW_INFO("0x%03x\t", i);
		for (j = 0; j < 8; j++)
	...
	while (idx < cnts) {
		...
		if (word_en != 0xF) {
			ret = EfusePgPacketWrite_BT(adapter, offset, word_en,
						    newdata, _FALSE);
			RTW_INFO("offset=%x\n", offset);
			RTW_INFO("word_en=%x\n", word_en);
...
VOID hal_ReadEFuse_BT_logic_map(PADAPTER padapter, u16 _offset, u16 _size_byte,
				u8 *pbuf)
{
	...
	if (rtw_efuse_bt_access(padapter, _FALSE, 0, EFUSE_BT_REAL_CONTENT_LEN,
				phyefuse))
	...
	for (bank = 1; bank <= total; bank++) { /* 8723d Max bake 0~2 */
		...
		while (AVAILABLE_EFUSE_ADDR(eFuse_Addr)) {
			...
			RTW_INFO("%s: efuse[%#X]=0x%02x (header)\n",
				 __FUNCTION__,
				 (((bank - 1) * EFUSE_BT_REAL_CONTENT_LEN) +
				  eFuse_Addr - 1),
				 efuseHeader);
			...
			if (EXT_HEADER(efuseHeader)) { /* extended header */
				...
				RTW_INFO(
				    "%s: extended header offset_2_0=0x%X\n",
				    __FUNCTION__, offset);

				/* ReadEFuseByte(padapter, eFuse_Addr++,
				 * &efuseExtHdr, bPseudoTest); */
				efuseExtHdr = phyefuse[eFuse_Addr++];

				RTW_INFO(
				    "%s: efuse[%#X]=0x%02x (ext header)\n",
				    __FUNCTION__,
				    (((bank - 1) * EFUSE_BT_REAL_CONTENT_LEN) +
				     eFuse_Addr - 1),
				    efuseExtHdr);
			...
			if (offset < EFUSE_BT_MAX_SECTION) {
				...
				for (i = 0; i < EFUSE_MAX_WORD_UNIT; i++) {
					...
					if (!(wden & (0x01 << i))) {
						efuseData = 0;
						/* ReadEFuseByte(padapter,
						 * eFuse_Addr++, &efuseData,
						 * bPseudoTest); */
						efuseData =
						    phyefuse[eFuse_Addr++];
...
static u8 hal_EfusePartialWriteCheck(PADAPTER padapter, u8 efuseType,
				     ...
				     u8 bPseudoTest)
{
	...
	u8 bRet = _FALSE;
	u16 startAddr = 0,
	    efuse_max_available_len = EFUSE_BT_REAL_BANK_CONTENT_LEN,
	    efuse_max = EFUSE_BT_REAL_BANK_CONTENT_LEN;
	u8 efuse_data = 0;
	...
	while (1) {
		if (startAddr >= efuse_max_available_len) {
			bRet = _FALSE;
			RTW_INFO("%s: startAddr(%d) >= "
				 "efuse_max_available_len(%d)\n",
				 __FUNCTION__, startAddr,
				 efuse_max_available_len);
			break;
		}
		if (rtw_efuse_bt_access(padapter, _FALSE, startAddr, 1,
					&efuse_data) &&
		    (efuse_data != 0xFF)) {
			bRet = _FALSE;
			RTW_INFO("%s: Something Wrong! last bytes(%#X=0x%02X) "
				 "is not 0xFF\n",
			...
			*pAddr = startAddr;
			/*			RTW_INFO("%s: Started from unused
			 * header offset=%d\n", __FUNCTION__, startAddr)); */
			bRet = _TRUE;
			break;
		}
...
static u8 hal_EfusePgPacketWrite2ByteHeader(PADAPTER padapter, u8 efuseType,
					    u16 *pAddr,
					    ...
					    u8 bPseudoTest)
{
	...
	do {
		...
		rtw_efuse_bt_access(padapter, _FALSE, efuse_addr, 1,
				    &tmp_header);
...
static u8 hal_EfusePgPacketWrite1ByteHeader(PADAPTER pAdapter, u8 efuseType,
					    u16 *pAddr,
					    PPGPKT_STRUCT pTargetPkt,
					    u8 bPseudoTest)
{
...
static u8 hal_EfusePgPacketWriteHeader(PADAPTER padapter, u8 efuseType,
				       u16 *pAddr, PPGPKT_STRUCT pTargetPkt,
				       u8 bPseudoTest)
{
	u8 bRet = _FALSE;
	...
	if (pTargetPkt->offset >= EFUSE_MAX_SECTION_BASE)
		bRet = hal_EfusePgPacketWrite2ByteHeader(
		    padapter, efuseType, pAddr, pTargetPkt, bPseudoTest);
	else
		bRet = hal_EfusePgPacketWrite1ByteHeader(
		    padapter, efuseType, pAddr, pTargetPkt, bPseudoTest);

	return bRet;
}

static u8 Hal_EfuseWordEnableDataWrite(PADAPTER padapter, u16 efuse_addr,
				       u8 word_en, u8 *data, u8 bPseudoTest)
{
	u16 tmpaddr = 0;
	...
	if (!(word_en & BIT(0))) {
		...
		rtw_efuse_bt_access(padapter, _FALSE, tmpaddr + 1, 1,
				    &tmpdata[1]);
...
static u8 hal_EfusePgPacketWriteData(PADAPTER pAdapter, u8 efuseType,
				     u16 *pAddr, PPGPKT_STRUCT pTargetPkt,
				     u8 bPseudoTest)
{
	u16 efuse_addr;
	...
	efuse_addr = *pAddr;
	badworden = Hal_EfuseWordEnableDataWrite(pAdapter, efuse_addr + 1,
						 pTargetPkt->word_en,
						 pTargetPkt->data, bPseudoTest);
	if (badworden != 0x0F) {
		RTW_INFO("%s: Fail!!\n", __FUNCTION__);
		return _FALSE;
	} else
...
static u16 hal_EfuseGetCurrentSize_BT(PADAPTER padapter, u8 bPseudoTest)
{
#ifdef HAL_EFUSE_MEMORY
	PHAL_DATA_TYPE pHalData = GET_HAL_DATA(padapter);
	PEFUSE_HAL pEfuseHal = &pHalData->EfuseHal;
	...
	for (bank = startBank; bank < 3; bank++) {
		...
		while (AVAILABLE_EFUSE_ADDR(efuse_addr)) {
			if (rtw_efuse_bt_access(padapter, _FALSE, efuse_addr, 1,
						&efuse_data) == _FALSE) {
			...
			RTW_INFO("%s: efuse_OneByteRead ! addr=0x%X "
				 "!efuse_data=0x%X! bank =%d\n",
			...
			if (EXT_HEADER(efuse_data)) {
				...
				efuse_addr++;
				rtw_efuse_bt_access(padapter, _FALSE,
						    efuse_addr, 1, &efuse_data);
				RTW_INFO(
				    "%s: efuse_OneByteRead EXT_HEADER ! "
				    "addr=0x%X !efuse_data=0x%X! bank =%d\n",
				    __FUNCTION__, efuse_addr, efuse_data, bank);
...
static u8 hal_BT_EfusePgCheckAvailableAddr(PADAPTER pAdapter, u8 bPseudoTest)
{
	u16 max_available =
	    EFUSE_BT_REAL_CONTENT_LEN - EFUSE_PROTECT_BYTES_BANK;
	u16 current_size = 0;
...
u8 EfusePgPacketWrite_BT(PADAPTER pAdapter, u8 offset, u8 word_en, u8 *pData,
			 u8 bPseudoTest)
{
	...
	if (!hal_EfusePartialWriteCheck(pAdapter, efuseType, &startAddr,
					&targetPkt, bPseudoTest))
		return _FALSE;

	if (!hal_EfusePgPacketWriteHeader(pAdapter, efuseType, &startAddr,
					  &targetPkt, bPseudoTest))
		return _FALSE;

	if (!hal_EfusePgPacketWriteData(pAdapter, efuseType, &startAddr,
					&targetPkt, bPseudoTest))
		return _FALSE;
...
BOOLEAN
Efuse_Read1ByteFromFakeContent(IN PADAPTER pAdapter, IN u16 Offset,
			       IN OUT u8 *Value);
BOOLEAN
Efuse_Read1ByteFromFakeContent(IN PADAPTER pAdapter, IN u16 Offset,
			       IN OUT u8 *Value)
...
BOOLEAN
Efuse_Write1ByteToFakeContent(IN PADAPTER pAdapter, IN u16 Offset, IN u8 Value);
BOOLEAN
Efuse_Write1ByteToFakeContent(IN PADAPTER pAdapter, IN u16 Offset, IN u8 Value)
{
	if (Offset >= EFUSE_MAX_HW_SIZE)
 ...
 *				pwr on state. When we stop write, we should switch to
 *500k mode
 ...
 *
 *---------------------------------------------------------------------------*/
VOID Efuse_PowerSwitch(IN PADAPTER pAdapter, IN u8 bWrite, IN u8 PwrState)
{
	pAdapter->hal_func.EfusePowerSwitch(pAdapter, bWrite, PwrState);
}

VOID BTEfuse_PowerSwitch(IN PADAPTER pAdapter, IN u8 bWrite, IN u8 PwrState)
{
	if (pAdapter->hal_func.BTEfusePowerSwitch)
		pAdapter->hal_func.BTEfusePowerSwitch(pAdapter, bWrite,
						      PwrState);
}
 ...
 *
 *---------------------------------------------------------------------------*/
u16 Efuse_GetCurrentSize(IN PADAPTER pAdapter, IN u8 efuseType,
			 IN BOOLEAN bPseudoTest)
{
	u16 ret = 0;

	ret = pAdapter->hal_func.EfuseGetCurrentSize(pAdapter, efuseType,
						     bPseudoTest);
		   ...
		   IN BOOLEAN bPseudoTest)
{
	...
	if (IS_HARDWARE_TYPE_8723B(Adapter)) {
		...
		/* 0x34[11]: SW force PGMEN input of efuse to high. (for the
		 * bank selected by 0x34[9:8]) */
	 ...
	 * condition. */
	/* Designer says that there shall be some delay after ready bit is set,
	 * or the */
 ...
 *	Created by Roger, 2008.10.21.
 *
 *	2008/12/12 MH	1. Reorganize code flow and reserve bytes. and add
 *description.
 *					2. Add efuse utilization collect.
 *	2008/12/22 MH	Read Efuse must check if we write section 1 data
 *again!!! Sec1
 *					write addr must be after sec5.
 *   */

VOID efuse_ReadEFuse(PADAPTER Adapter, u8 efuseType, u16 _offset,
		     u16 _size_byte, u8 *pbuf, IN BOOLEAN bPseudoTest);
VOID efuse_ReadEFuse(PADAPTER Adapter, u8 efuseType, u16 _offset,
		     u16 _size_byte, u8 *pbuf, IN BOOLEAN bPseudoTest)
{
	Adapter->hal_func.ReadEFuse(Adapter, efuseType, _offset, _size_byte,
				    pbuf, bPseudoTest);
}

VOID EFUSE_GetEfuseDefinition(IN PADAPTER pAdapter, IN u8 efuseType, IN u8 type,
			      OUT void *pOut, IN BOOLEAN bPseudoTest)
{
	pAdapter->hal_func.EFUSEGetEfuseDefinition(pAdapter, efuseType, type,
						   pOut, bPseudoTest);
}

/*  11/16/2008 MH Read one byte from real Efuse. */
u8 efuse_OneByteRead(IN PADAPTER pAdapter, IN u16 addr, IN u8 *data,
		     IN BOOLEAN bPseudoTest)
{
	u32 tmpidx = 0;
	u8 bResult;
	...
	if (IS_HARDWARE_TYPE_8710B(pAdapter)) {
		bResult = pAdapter->hal_func.efuse_indirect_read4(pAdapter,
								  addr, data);
		return bResult;
	}
#endif
	...
	if (IS_HARDWARE_TYPE_8723B(pAdapter) ||
		...
		rtw_write16(pAdapter, 0x34,
			    rtw_read16(pAdapter, 0x34) & (~BIT11));
	}
	...
	if (tmpidx < 100) {
		...
		*data = 0xff;
		...
		RTW_INFO("%s: [ERROR] EFUSE_CTRL =0x%08x !!!\n", __FUNCTION__,
			 rtw_read32(pAdapter, EFUSE_CTRL));
	}
...
u8 efuse_OneByteWrite(IN PADAPTER pAdapter, IN u16 addr, IN u8 data,
		      IN BOOLEAN bPseudoTest)
{
	u8 tmpidx = 0;
	u8 bResult = _FALSE;
	...
	/* <20130227, Kordan> 8192E MP chip A-cut had better not set 0x34[11]
	 * until B-Cut. */
	if (IS_HARDWARE_TYPE_8723B(pAdapter) ||
	    (IS_HARDWARE_TYPE_8192E(pAdapter) &&
	     (!IS_A_CUT(pHalData->version_id))) ||
		...
		rtw_write16(pAdapter, 0x34,
			    rtw_read16(pAdapter, 0x34) | (BIT11));
		rtw_write32(pAdapter, EFUSE_CTRL,
			    0x90600000 | ((addr << 8 | data)));
	} else
	...
	if (tmpidx < 100)
		...
		bResult = _FALSE;
		RTW_INFO("%s: [ERROR] addr=0x%x ,efuseValue=0x%x ,bResult=%d "
			 "time out 1s !!!\n",
...
int Efuse_PgPacketRead(IN PADAPTER pAdapter, IN u8 offset, IN u8 *data,
		       IN BOOLEAN bPseudoTest)
{
	int ret = 0;

	ret = pAdapter->hal_func.Efuse_PgPacketRead(pAdapter, offset, data,
						    bPseudoTest);

	return ret;
}

int Efuse_PgPacketWrite(IN PADAPTER pAdapter, IN u8 offset, IN u8 word_en,
			IN u8 *data, IN BOOLEAN bPseudoTest)
{
	int ret;

	ret = pAdapter->hal_func.Efuse_PgPacketWrite(pAdapter, offset, word_en,
						     data, bPseudoTest);

	return ret;
}

int Efuse_PgPacketWrite_BT(IN PADAPTER pAdapter, IN u8 offset, IN u8 word_en,
			   IN u8 *data, IN BOOLEAN bPseudoTest)
{
	int ret;

	ret = pAdapter->hal_func.Efuse_PgPacketWrite_BT(
	    pAdapter, offset, word_en, data, bPseudoTest);

	return ret;
}

u8 Efuse_WordEnableDataWrite(IN PADAPTER pAdapter, IN u16 efuse_addr,
			     IN u8 word_en, IN u8 *data, IN BOOLEAN bPseudoTest)
{
	u8 ret = 0;

	ret = pAdapter->hal_func.Efuse_WordEnableDataWrite(
	    pAdapter, efuse_addr, word_en, data, bPseudoTest);
 ...
 */
u8 rtw_efuse_access(PADAPTER padapter, u8 bWrite, u16 start_addr, u16 cnts,
		    u8 *data)
{
	int i = 0;
	...
	EFUSE_GetEfuseDefinition(padapter, EFUSE_WIFI,
				 ...
				 (PVOID)&real_content_len, _FALSE);
	EFUSE_GetEfuseDefinition(padapter, EFUSE_WIFI,
				 TYPE_AVAILABLE_EFUSE_BYTES_TOTAL,
				 (PVOID)&max_available_size, _FALSE);
...
u8 rtw_BT_efuse_map_read(PADAPTER padapter, u16 addr, u16 cnts, u8 *data)
{
	...
	EFUSE_GetEfuseDefinition(padapter, EFUSE_BT, TYPE_EFUSE_MAP_LEN,
				 (PVOID)&mapLen, _FALSE);
...
#define RT_ASSERT_RET(expr)                                                    \
	...
	EFUSE_GetEfuseDefinition(padapter, EFUSE_WIFI,
				 TYPE_AVAILABLE_EFUSE_BYTES_TOTAL,
				 &efuse_max_available_len, _FALSE);
	...
	while (idx < cnts) {
		...
		if (word_en != 0xF) {
			...
			if (offset >=
			    EFUSE_MAX_SECTION_BASE) /* Over EFUSE_MAX_SECTION 16
						       for 2 ByteHeader */
				chk_total_byte += 2;
			else
				chk_total_byte += 1;
	...
	if ((startAddr + chk_total_byte) >= efuse_max_available_len) {
		RTW_INFO("%s: startAddr(0x%X) + PG data len %d >= "
			 "efuse_max_available_len(0x%X)\n",
			 __func__, startAddr, chk_total_byte,
			 efuse_max_available_len);
		ret = _FAIL;
		goto exit;
	}
...
#ifdef CONFIG_RTL8723B
				if (addr + idx == 0x8) {
					if (IS_C_CUT(pHalData->version_id) ||
					    IS_B_CUT(pHalData->version_id)) {
						if (pHalData
							->adjuseVoltageVal ==
						    6) {
							newdata[i] =
...
#define RT_ASSERT_RET(expr)                                                    \
	...
	if (ret == _FAIL)
		goto exit;
	RTW_INFO("OFFSET\tVALUE(hex)\n");
	for (i = 0; i < 1024; i += 16) { /* set 512 because the iwpriv's extra
					    size have limit 0x7FF */
		RTW_INFO("0x%03x\t", i);
		for (j = 0; j < 8; j++)
			RTW_INFO("%02X ", map[i + j]);
		RTW_INFO("\t");
	...
	while (idx < cnts) {
		...
		if (word_en != 0xF) {
			...
			for (i = 0; i < PGPKT_DATA_SIZE; i++)
				RTW_INFO("0x%02X ", newdata[i]);
			RTW_INFO("\n");
			ret = Efuse_PgPacketWrite_BT(padapter, offset, word_en,
						     newdata, _FALSE);
			if (ret == _FAIL)
				break;
		}
 ...
 *
 *---------------------------------------------------------------------------*/
VOID Efuse_ReadAllMap(IN PADAPTER pAdapter, IN u8 efuseType, IN OUT u8 *Efuse,
		      IN BOOLEAN bPseudoTest);
VOID Efuse_ReadAllMap(IN PADAPTER pAdapter, IN u8 efuseType, IN OUT u8 *Efuse,
		      IN BOOLEAN bPseudoTest)
{
 ...
 *---------------------------------------------------------------------------*/
#ifdef PLATFORM
static VOID efuse_ShadowWrite1Byte(IN PADAPTER pAdapter, IN u16 Offset,
				   IN u8 Value);
#endif /* PLATFORM */
static VOID efuse_ShadowWrite1Byte(IN PADAPTER pAdapter, IN u16 Offset,
				   IN u8 Value)
...
/* ---------------Write Two Bytes */
static VOID efuse_ShadowWrite2Byte(IN PADAPTER pAdapter, IN u16 Offset,
				   IN u16 Value)
{
...
/* ---------------Write Four Bytes */
static VOID efuse_ShadowWrite4Byte(IN PADAPTER pAdapter, IN u16 Offset,
				   IN u32 Value)
{
	PHAL_DATA_TYPE pHalData = GET_HAL_DATA(pAdapter);
 ...
 *
 *---------------------------------------------------------------------------*/
VOID EFUSE_ShadowWrite(IN PADAPTER pAdapter, IN u8 Type, IN u16 Offset,
		       IN OUT u32 Value);
VOID EFUSE_ShadowWrite(IN PADAPTER pAdapter, IN u8 Type, IN u16 Offset,
		       IN OUT u32 Value)
{
 ...
 *
 *---------------------------------------------------------------------------*/
static VOID efuse_ShadowRead1Byte(IN PADAPTER pAdapter, IN u16 Offset,
				  IN OUT u8 *Value)
{
	PHAL_DATA_TYPE pHalData = GET_HAL_DATA(pAdapter);
...
/* ---------------Read Two Bytes */
static VOID efuse_ShadowRead2Byte(IN PADAPTER pAdapter, IN u16 Offset,
				  IN OUT u16 *Value)
{
	PHAL_DATA_TYPE pHalData = GET_HAL_DATA(pAdapter);
...
/* ---------------Read Four Bytes */
static VOID efuse_ShadowRead4Byte(IN PADAPTER pAdapter, IN u16 Offset,
				  IN OUT u32 *Value)
{
	PHAL_DATA_TYPE pHalData = GET_HAL_DATA(pAdapter);
 ...
 *---------------------------------------------------------------------------*/
void EFUSE_ShadowRead(IN PADAPTER pAdapter, IN u8 Type, IN u16 Offset,
		      IN OUT u32 *Value)
{
	if (Type == 1)
...
u8 Efuse_CalculateWordCnts(IN u8 word_en)
{
	u8 word_cnts = 0;
	if (!(word_en & BIT(0)))
 ...
 *
 *---------------------------------------------------------------------------*/
void efuse_WordEnableDataRead(IN u8 word_en, IN u8 *sourdata, IN u8 *targetdata)
{
	if (!(word_en & BIT(0))) {
 ...
 *
 *---------------------------------------------------------------------------*/
void EFUSE_ShadowMapUpdate(IN PADAPTER pAdapter, IN u8 efuseType,
			   IN BOOLEAN bPseudoTest)
{
	PHAL_DATA_TYPE pHalData = GET_HAL_DATA(pAdapter);
	u16 mapLen = 0;
	...
	if (mapLen > EEPROM_MAX_SIZE) {
		RTW_WARN("%s: <ERROR> size of efuse data(%d) is large than "
			 "expected(%d)!\n",
	...
	if (pHalData->bautoload_fail_flag == _FALSE) {
		err = rtw_halmac_read_logical_efuse_map(
		    adapter_to_dvobj(pAdapter), efuse_map, mapLen, NULL, 0);
		if (err)
...
const u8 _mac_hidden_max_bw_to_hal_bw_cap[MAC_HIDDEN_MAX_BW_NUM] = {
    0,
    0,
    (BW_CAP_160M | BW_CAP_80M | BW_CAP_40M | BW_CAP_20M | BW_CAP_10M |
     BW_CAP_5M),
    (BW_CAP_5M),
...
const u8 _mac_hidden_proto_to_hal_proto_cap[MAC_HIDDEN_PROTOCOL_NUM] = {
    0, 0, (PROTO_CAP_11N | PROTO_CAP_11G | PROTO_CAP_11B),
...
u8 rtw_efuse_file_read(PADAPTER padapter, u8 *filepatch, u8 *buf, u32 len)
{
	...
	while ((j < len) && (i < count)) {
		...
		if (err != 1) {
			RTW_WARN(
			    "Something wrong to parse efuse file, string=%s\n",
			    &ptmpbuf[i]);
		} else {
			buf[j] = val8;
...
u32 rtw_read_efuse_from_file(const char *path, u8 *buf, int map_size)
{
	...
	for (i = 0; i < map_size; i++) {
		...
		while (1) {
			...
			if (temp_i == 0 &&
			    (is_eol(c) == _TRUE || is_space(c) == _TRUE ||
			     is_null(c) == _TRUE))
				continue;

			if (IsHexDigit(c) == _FALSE) {
				RTW_ERR("%s invalid 8-bit hex format for "
					"offset:0x%03x\n",
					__func__, i);
				goto exit;
			}
		...
		if (end == _TRUE) {
			if (temp_i != 0) {
				RTW_ERR("%s incomplete 8-bit hex format for "
					"offset:0x%03x\n",
					__func__, i);
				goto exit;
			}
...
u32 rtw_read_macaddr_from_file(const char *path, u8 *buf)
{
	...
	for (i = 0; i < ETH_ALEN; i++) {
		if (IsHexDigit(file_data[i * 3]) == _FALSE ||
		    IsHexDigit(file_data[i * 3 + 1]) == _FALSE) {
			RTW_ERR("%s invalid 8-bit hex format for address "
				"offset:%u\n",
				__func__, i);
			goto exit;
		}
		...
		if (i < ETH_ALEN - 1 && file_data[i * 3 + 2] != ':') {
			RTW_ERR(
			    "%s invalid separator after address offset:%u\n",
			    __func__, i);
			goto exit;
		}
