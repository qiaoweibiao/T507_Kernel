...
static inline void schedule_tx_task(void)
{
...
static int sprdwl_tx_work_queue(void *data)
{
	...
	while (!intf->exit || !kthread_should_stop()) {
		need_polling = 0;
		polling_times = 0;
		...
		*/
		...
		if (need_polling) {
			...
			polling_times = 0;
			while (!sprdwl_msg_tx_pended(&tx_msg->tx_list_cmd) &&
				(atomic_read(&tx_msg->tx_list_qos_pool.ref) <= 16) &&
