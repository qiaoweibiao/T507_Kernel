...
#define MARLIN_AD_CHIPID 0x23550003
	...
	switch (ant) {
	case MARLIN_TWO_ANT:
		strcat(path_buf, "two_wire_ant/");
		break;

	case MARLIN_THREE_ANT:
		strcat(path_buf, "three_wire_ant/");
		break;

	default:
		wl_err("%s,Use default ant config(ant:%d)\n", __func__, ant);
		break;
	}*/
	...
	case MARLIN_AB_CHIPID:
		strcat(path_buf, "wifi_board_config_ab.ini");
		break;
	case MARLIN_AC_CHIPID:
		strcat(path_buf, "wifi_board_config_ac.ini");
		break;
	default:
		strcat(path_buf, "wifi_board_config.ini");
		break;
	}
