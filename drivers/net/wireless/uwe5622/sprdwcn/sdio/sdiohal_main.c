...
unsigned char sdiohal_get_tx_mode(void)
{
	...
	return ((p_data->adma_tx_enable == true) ? 1 : 0);
}

unsigned char sdiohal_get_rx_mode(void)
{
	struct sdiohal_data_t *p_data = sdiohal_get_data();
	return ((p_data->adma_rx_enable == true) ? 1 : 0);
}
...
void sdiohal_set_carddump_status(unsigned int flag)
{
	...
	if (flag == true) {
		if (!p_data->inband_irq && p_data->irq_num)
...
static int sdiohal_parse_dt(void)
{
	...
	p_data->adma_tx_enable = wcn_port.cfg_adma_tx;
	p_data->adma_rx_enable = wcn_port.cfg_adma_rx;

	WCN_INFO("adma enable tx:%d, rx:%d\n",
		 p_data->adma_tx_enable, p_data->adma_rx_enable);

	p_data->pwrseq_enable = wcn_port.cfg_pwrseq;

	WCN_INFO("%s sdio pwrseq enable:%d\n",
		 __func__, p_data->pwrseq_enable);

	p_data->inband_irq = wcn_port.cfg_data_irq;

	WCN_INFO("%s sdio data[1] interrupt:%d\n",
		 __func__, p_data->inband_irq);

	p_data->gpio_num = wcn_port.pin_wakeup_ap;
	sdio_bus = wcn_port.sdio_bus;

	sprintf(match, "sdc%d", sdio_bus);
	sdio_node = of_find_node_by_type(NULL, match);
	if (sdio_node == NULL) {
...
static int sdiohal_suspend(struct device *dev)
{
	...
	if (p_data->inband_irq) {
#ifndef CONFIG_SDIO_INBAND_POLLING
		sdio_claim_host(p_data->sdio_func[FUNC_1]);
		sdio_release_irq(p_data->sdio_func[FUNC_1]);
		sdio_release_host(p_data->sdio_func[FUNC_1]);
#endif
	} else if (p_data->irq_num != 0) {
		disable_irq(p_data->irq_num);
	}
...
static int sdiohal_resume(struct device *dev)
{
	...
	if (p_data->inband_irq) {
		...
		if (sdio_claim_irq(p_data->sdio_func[FUNC_1],
			...
			WCN_ERR("%s: Failed to request IRQ\n",
				__func__);
			sdio_release_host(p_data->sdio_func[FUNC_1]);
			return -1;
		}
...
int sdiohal_runtime_get(void)
{
	...
	/* pwrseq_enable is false,here not change inband_irq */
	if (!p_data->inband_irq) {
		enable_irq(p_data->irq_num);
	} else {
		sdio_claim_host(p_data->sdio_func[FUNC_1]);
		if (sdio_claim_irq(p_data->sdio_func[FUNC_1],
				   sdiohal_irq_handler_data)) {
			WCN_ERR("%s,%d: Failed to request IRQ\n",
				__func__, __LINE__);
			sdio_release_host(p_data->sdio_func[FUNC_1]);
			return -1;
		}
