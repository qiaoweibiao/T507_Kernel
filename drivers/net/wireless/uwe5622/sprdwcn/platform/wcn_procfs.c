...
#include <config/generic/sched/clock.h>
...
/* for test power on/off frequently */
	...
	if (strncmp(mdbg_proc->write_buf, "logpath=",
		...
		/* If change log path by at cmd, NOT cover old log files. */
		wcn_set_log_file_cover_old(0);

		return count;
	}

	if (strncmp(mdbg_proc->write_buf, "at+armlog=1", 11) == 0) {
		WCN_INFO("%s: init log file path, if neccessary\n", __func__);
		wcn_debug_init();
	}
