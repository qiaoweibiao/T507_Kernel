/*
 * A V4L2 driver for rn6864m cameras and AHD Coax protocol.
 *
...
MODULE_AUTHOR("cwh");
MODULE_DESCRIPTION("A low-level driver for rn6864m mipi chip for cvbs sensor");
MODULE_LICENSE("GPL");
...
#define V4L2_IDENT_SENSOR  0x05// 0x05   

/*
 * Our nominal (default) frame rate.
 */
#define SENSOR_FRAME_RATE	25//25

/*
 * The rn6864m i2c address
 */

#define I2C_ADDR  0x5a //0x5a,0x2c,0x2d 8bit address

/*static struct delayed_work sensor_s_ae_ratio_work;*/
static bool restart;
#define SENSOR_NAME "rn6864m_mipi"
...
static struct regval_list sensor_1080p_30fps_regs[] = {
	...
	{0x00, 0x20}, // internal use*
	{0x06, 0x08}, // internal use*
	{0x07, 0x63}, // HD format 0x63
	{0x2A, 0x00}, // filter control 0x01
	{0x3A, 0x20}, // Insert Channel ID in SAV/EAV code
	{0x3F, 0x10}, // channel ID
	{0x4C, 0x37}, // equalizer
	{0x4F, 0x03}, // sync control
	...
	{0x28, 0x92}, // cropping
	{0x03, 0x40}, // saturation 0x80 0x40
	{0x04, 0x80}, // hue 0x80
	{0x05, 0x08}, // sharpness 0x04 
	{0x57, 0x22}, // black/white stretch 0x23
	{0x68, 0x01}, // coring  0x00
	{0x37, 0x33},
	{0x61, 0x6C},
	...
	{0x04, 0x00}, // csi1 and tx1 reset finish
	//{0x07, 0x05},//enable non-continuous clock 非连续模式
	{0x20, 0xAA}, // invert hs clock

	// mipi link3
	{0xFF, 0x0A}, // switch to mipi csi3 qwb
	{0x6C, 0x10}, // disable ch output; turn off ch0~3 
};
...
static struct regval_list sensor_1080p_25fps_regs[] = {
	...
	{0x28, 0x92}, // cropping 0x92  

	{0x03, 0x90}, // saturation 0x80 0x40 有效
	{0x04, 0x80}, // hue 0x80
	{0x05, 0x04}, // sharpness 0x04  无效
	{0x57, 0x23}, // black/white stretch 0x23 有效
	{0x68, 0x00}, // coring  0x00
	{0x37, 0x33},
	{0x61, 0x6C},
...
static int sensor_power(struct v4l2_subdev *sd, int on)
{
	...
	case PWR_ON://qwb007
		...
		vin_set_mclk_freq(sd, MCLK);
		vin_set_mclk(sd, ON);
		usleep_range(1000, 1200);//usleep_range(1000, 1200); usleep_range(100, 120);
		...
		//vin_gpio_write(sd, RESET, CSI_GPIO_HIGH);//CSI_GPIO_LOW
...
static int sensor_init(struct v4l2_subdev *sd, u32 val)
{
	...
	info->low_speed = 0;
	info->width = 1920;    
	info->height = 1080;//1080
	info->hflip = 0;//0
	info->vflip = 0;//0
	info->gain = 0;
	info->tpf.numerator = 1;//0
	info->tpf.denominator = 25;//25
...
static struct sensor_format_struct sensor_formats[] = {
	{
		...
		.regs 		= sensor_1080p_25fps_regs,// sensor_default_regs
		.regs_size = ARRAY_SIZE(sensor_1080p_25fps_regs),//sensor_default_regs
		.bpp		= 2,//2
	}
};
...
static struct sensor_win_size sensor_win_sizes[] = {
	  ...
	  .hoffset = 0,
	  .voffset = 0,
	  .pclk = 2*567*1000*1000,//567*1000*1000
	  .mipi_bps = 2*567*1000*1000,
	  .fps_fixed = 25,//30
