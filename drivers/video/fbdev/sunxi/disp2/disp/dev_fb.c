...
int show_fb_to_ui(void)
{
	...
	for (fb_id =0;fb_id<SUNXI_FB_MAX;fb_id++){
		chan = g_fbi.layer_hdl[fb_id][0];
		layer_id = g_fbi.layer_hdl[fb_id][1];
		if((chan == 1)&&(layer_id==0)){
			printk("-----fb%d UI0 \r\n",fb_id);

		}else if((chan == 2)&&(layer_id==0)){
...
EXPORT_SYMBOL(show_fb_to_ui);

static int move_disp_to_invaild(int fbid,int chan,int lid)
{//if other fb used this ly,change it to other.
	int fb_id;
	for (fb_id =0;fb_id<SUNXI_FB_MAX;fb_id++){
		if(((g_fbi.layer_hdl[fb_id][0]==chan)&&(
		g_fbi.layer_hdl[fb_id][1]==lid))/*||(fb_id == fbid)*/){
			
			g_fbi.layer_hdl[fb_id][0] = 3;
			g_fbi.layer_hdl[fb_id][1] = 0;
			g_fbi.config[fb_id].info.zorder = FB2_ZORDER;
			g_fbi.config[fb_id].info.alpha_mode = 1;
			g_fbi.config[fb_id].info.alpha_value = 0;
			//g_fbi.config[fb_id].alpha_mode = 0;
...
static void printf_fb_parm(void)
{
	int fb_id;
	for (fb_id =0;fb_id<SUNXI_FB_MAX;fb_id++){
		pr_warn("fb%d en:%d ch[%d]ly[%d]z[%d] fmt:%d add[%llx] \r\n",fb_id,
			g_fbi.config[fb_id].enable,
			g_fbi.config[fb_id].channel,
			g_fbi.config[fb_id].layer_id,
			g_fbi.config[fb_id].info.zorder,
			g_fbi.config[fb_id].info.fb.format,
			g_fbi.config[fb_id].info.fb.addr[0]);
	}
}
int sunxi_fb_to_display(int fb_id,int UIid,int enable)
{
	u32 sel = 0;
	u32 num_screens = 0;
	s32 chan = 0;
	...
	struct disp_layer_config config1;
	
	if(UIid==0)
	{
		chan = 1;//g_fbi.layer_hdl[fb_id][0];
		layer_id = 0;//= g_fbi.layer_hdl[fb_id][1];
		
	}else	if(UIid==1)
	{
		chan = 2;//g_fbi.layer_hdl[fb_id][0];
		layer_id = 0;//= g_fbi.layer_hdl[fb_id][1];
	}else if(UIid==2)
	{
		chan = 3;//g_fbi.layer_hdl[fb_id][0];
		layer_id = 0;//= g_fbi.layer_hdl[fb_id][1];
	}
	else
	{
		__wrn("done support uiid =%d \r\n",UIid);
		return -1;
	}
	if((g_fbi.layer_hdl[fb_id][0] == chan)&&
	(g_fbi.layer_hdl[fb_id][1] == layer_id)){
		__wrn("fb%d is already use ui%d,no need to change.\r\n",fb_id,UIid);
		return 0;
	}
	__wrn("sunxi_fb_to_display fb%d UIid %d \r\n",fb_id,UIid);
	printf_fb_parm();
    
	chanback = g_fbi.layer_hdl[fb_id][0];
	layer_idback = g_fbi.layer_hdl[fb_id][1];

	if(((chanback == 1)&&(layer_idback==0))||
		((chanback == 2)&&(layer_idback==0))||
		((chanback == 3)&&(layer_idback==0))){
		if((chan!=chanback)||(layer_idback!=layer_id)){
			needclear =1;//if this fb use ui0&ui1&ui2,close it.
		}
	}
		
	//if other fb used this ly,change it to other.
	move_disp_to_invaild(fb_id,chan,layer_id);
	
	g_fbi.layer_hdl[fb_id][0]=chan;
	g_fbi.layer_hdl[fb_id][1]=layer_id;	
	g_fbi.config[fb_id].info.alpha_mode = 0;
	g_fbi.config[fb_id].info.alpha_value = 0;
	...
	for (sel = 0; sel < num_screens; sel++) {
		if (sel==g_fbi.fb_mode[fb_id]) {
			...
			if (mgr && mgr->get_layer_config && mgr->set_layer_config) {
				if(needclear){
					//if(fb_id == 0)
						//memcpy(&config,&aut_config[1], sizeof(struct disp_layer_config));//must cpyf 1
					config1.channel = chanback;
					config1.layer_id = layer_idback;
					config1.enable =0;
					
					if((chanback == 1)&&(layer_idback==0)){
						config1.info.zorder = FB0_ZORDER;
					}else if((chanback == 2)&&(layer_idback==0)){
						config1.info.zorder = FB1_ZORDER;
					}else{
						config1.info.zorder = FB2_ZORDER;
					}
					__wrn("needclear fb%d UIid %d chan[%d]lid[%d]z[%d]\r\n",fb_id,UIid,chanback,layer_idback,config1.info.zorder);
					if (0 != mgr->set_layer_config(mgr, &config1, 1)) {
						__wrn("fb %d, set_layer_config(%d,%d,%d) fail\n",fb_id, sel, chan, layer_id);
						return -1;
					}
				}
				memcpy(&config,&g_fbi.config[fb_id], sizeof(struct disp_layer_config));
				config.channel = chan;
				config.layer_id = layer_id;
				config.enable =1;
				g_fbi.config[fb_id].enable =1;
				if((chan == 1)&&(layer_id==0)){
					config.info.zorder = FB0_ZORDER;
					g_fbi.config[fb_id].info.zorder = FB0_ZORDER;
				}else if((chan == 2)&&(layer_id==0)){
					config.info.zorder = FB1_ZORDER;
					g_fbi.config[fb_id].info.zorder = FB1_ZORDER;
				}else
				{
					config.info.zorder = FB2_ZORDER;
					g_fbi.config[fb_id].info.zorder = FB2_ZORDER;
				}
				__wrn("sunxi_fb_to_display set fb%d UIid %d ch[%d]ly[%d]z[%d]\r\n",fb_id,UIid,chan,layer_id,config.info.zorder);
				//if (0 != mgr->get_layer_config(mgr, &config, 1)) {
				//	__wrn("fb %d, get_layer_config(%d,%d,%d) fail\n", fb_id, sel, chan, layer_id);
				//	return -1;
				//}
				if(((chan == 1)&&(layer_id==0))||((chan == 2)&&(layer_id==0))||((chan == 3)&&(layer_id==0))){
...
EXPORT_SYMBOL(sunxi_fb_to_display);
				...
				struct fb_info *info)
{
	...
	for (sel = 0; sel < num_screens; sel++) {
		if (sel == g_fbi.fb_mode[info->node]) {
			...
			struct disp_layer_config* config = &g_fbi.config[info->node];
			struct disp_manager *mgr = g_disp_drv.mgr[sel];
		...
		if (sel == g_fbi.fb_mode[info->node]) {
			...
			if (blank_mode == FB_BLANK_POWERDOWN) {
				if (mgr && mgr->get_layer_config
					...
					//memcpy(&g_fbi.config[sel], &config,
...
static s32 display_fb_request(u32 fb_id, struct disp_fb_create_info *fb_para)
{
	...
	g_fbi.layer_hdl[fb_id][1] = 0;
#else
	/* fb bound to layer(3,0)  */
	if(fb_id<3){
		g_fbi.layer_hdl[fb_id][0] = fb_id+1;
		g_fbi.layer_hdl[fb_id][1] = 0;
	}
	else{
		g_fbi.layer_hdl[fb_id][0] = 3;
	...
	memcpy(&g_fbi.config[fb_id], &config,
	       sizeof(struct disp_layer_config));
    __inf("init g_fbi.config[%d]addr=%llx\n",fb_id,g_fbi.config[fb_id].info.fb.addr[0]);
OUT:
	return ret;
}
...
s32 fb_init(struct platform_device *pdev)
{
	...
	if (g_disp_drv.disp_init.b_init) {
		...
		for (i = 0; i < SUNXI_FB_MAX; i++) {
			...
			disp_fb_to_var(disp_init->format[screen_id],
				       &(g_fbi.fbinfo[i]->var));
			fb_para.buffer_num = g_disp_drv.disp_init.buffer_num[screen_id];
			if ((disp_init->fb_width[screen_id] == 0)
			    || (disp_init->fb_height[screen_id] == 0)) {
				fb_para.width =
				...
				fb_para.width = disp_init->fb_width[screen_id];
				fb_para.height = disp_init->fb_height[screen_id];
			}
