...
struct force_layer_t {
	int _force_layer_en;
	int _force_layer_type;
};
...
struct disp_manager_private_data {
	...
	struct force_layer_t force_layer;
};
...
struct dmabuf_item back_dma_item[2][16];
...
static s32 disp_mgr_force_set_layer_config(struct disp_manager *mgr, struct disp_layer_config *config, unsigned int layer_num)
{
	...
	mgrp->force_layer._force_layer_en = 1;
...
static s32 disp_mgr_force_set_layer_config_exit(struct disp_manager *mgr)
{
	...
	if (layer_num <= 0 || layer_num > num_layers) {
		DE_WRN("invalid input params!:layer_num:%d num_layers:%d\n", layer_num, num_layers);
		mutex_unlock(&mgr_mlock);
		return -1;
	}
	if (mgrp->force_layer._force_layer_type == 1) {
		disp_mgr_set_layer_config2_restore(mgr, backup2, layer_num);
		mgrp->force_layer._force_layer_en = 0;
		mutex_unlock(&mgr_mlock);
		return DIS_SUCCESS;
	}
			  ...
			  unsigned int layer_num)
{
	...
	DE_INF
	    ("layer:ch%d, layer=%d,zorder=%d format=%d, size=<%d,%d>, crop=<%lld,%lld,%lld,%lld>,frame=<%d,%d>, en=%d addr[0x%llx,0x%llx,0x%llx> alpha=<%d,%d>\n",
	     config->channel, config->layer_id,config->info.zorder, config->info.fb.format,
	     config->info.fb.size[0].width, config->info.fb.size[0].height,
	     config->info.fb.crop.x >> 32, config->info.fb.crop.y >> 32,
	     config->info.fb.crop.width >> 32,
	     config->info.fb.crop.height >> 32, config->info.screen_win.width,
	...
	mutex_lock(&mgr_mlock);
	{
		...
		backup_layer_num[mgr->disp] =
		    (layer_num > num_layers) ? num_layers : layer_num;
		...
		if (mgrp->force_layer._force_layer_en) {
			mgrp->force_layer._force_layer_type = 0;
			mutex_unlock(&mgr_mlock);
			return DIS_SUCCESS;
		}
	}
			  ...
			  unsigned int layer_num)
{
		...
		for (i = 0; i < layer_num; i++, lyr_cfg++) {
			...
			item = &back_dma_item[mgr->disp]
					     [lyr_cfg->config.channel * 4 +
					      lyr_cfg->config.layer_id];
			  ...
			  unsigned int layer_num)
{
...

	{
		...
		mgrp->force_layer._force_layer_type = 1;
	}
	for (i = 0; i < layer_num; i++) {
		struct disp_layer *lyr = NULL;
	...
	for (i = 0, lyr_cfg = lyr_cfg_using[0]; i < layer_num; i++, lyr_cfg++) {
		...
		memcpy(&back_dma_item[mgr->disp][lyr_cfg->config.channel * 4 +
						 lyr_cfg->config.layer_id],
		       item, sizeof(struct dmabuf_item));
