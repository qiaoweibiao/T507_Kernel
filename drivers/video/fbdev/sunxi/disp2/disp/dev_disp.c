...
static ssize_t disp_fb2ui_show(struct device *dev,
    struct device_attribute *attr, char *buf)
{
	extern int show_fb_to_ui(void);
...
static ssize_t disp_fb2ui_store(struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long fb_val,ui_val;

	extern int sunxi_fb_to_display(int fb_id,int UIid,int enable);

	if (__parse_dump_str(buf, count, &fb_val, &ui_val)) {
		pr_err("%s,%d err, invalid para!\n", __func__, __LINE__);
		return -1;
	}

	printk(" fbid=%ld uiid=%ld\n",fb_val, ui_val);
	sunxi_fb_to_display(fb_val,ui_val,0);
...
static DEVICE_ATTR(fb2ui, 0660,    disp_fb2ui_show, disp_fb2ui_store);
