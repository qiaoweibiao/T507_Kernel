...
static void stk3x1x_init_work(struct work_struct *work)
{
	int ret;

	ret = i2c_add_driver(&stk_ps_driver);
	if (ret < 0) {
		printk("add stk3x1x i2c driver failed\n");
	}
...
static int __init stk3x1x_init(void)
{
	...
	init_wq = create_singlethread_workqueue("stk3x1x_init");
	if (init_wq == NULL) {
		printk("stk3x1x_init wq create failed, do it now\n");
		return i2c_add_driver(&stk_ps_driver);
	}
	INIT_WORK(&init_work, stk3x1x_init_work);
	queue_work(init_wq, &init_work);
	return 0;
}
