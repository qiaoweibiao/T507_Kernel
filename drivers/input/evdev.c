...
static struct input_mgr  input_mgr_proc[FOCUS_PROC_MAX] = { {{0}, 0, -1}, };
...
#define INPUT_CTRL_LIST_NUM 3
char g_ctrl_list[INPUT_CTRL_LIST_NUM][64] ={
	"Launcher",
	"MediaUI",
	"CameraUI"
	};
 ...
 //return 0 means procName is in the g_ctrl_list(we should not block the input data)
static int checkProcName(char * procName){
	int ret = -1;
	int i = 0;
	...
	for(i=0; i<INPUT_CTRL_LIST_NUM; i++){
		ret = strcmp(procName, &g_ctrl_list[i][0]);
		if(0 == ret){
			return ret;
		}
	}
			  ...
			  size_t count, loff_t *ppos)
{
	...
	ret = checkProcName(group_leader->comm);
	if(ret == 0){
		//printk("ProcName is in ctrl_list\n");
	}else{
		//printk("ProcName isn't in ctrl_list,let it go\n");
		return read;
	}
	
	for (i=0; i<FOCUS_PROC_MAX; i++){
	    if(strstr(input_mgr_proc[i].procname, group_leader->comm) 
			 		|| current->tgid == input_mgr_proc[i].pid ){	
			
			if(input_mgr_proc[i].enable == 1){
			    enable = 1;
			}
	    }
 	}
    ...
    struct device_attribute *attr, char *buf)
{
	...
	for(i=0;i<FOCUS_PROC_MAX;i++){
		if(input_mgr_proc[i].enable == 1){
			count = snprintf(data, sizeof(data), "%s %s\n", "input procname is ", input_mgr_proc[i].procname);
            if(count>128){                
                count = 127;
                data[127] = 0;
            }
    ...
    if((count >= 0)&&(count <=127)){
        ...
        printk("data len overflow,count = %d\n",count);
        count = 0;
    }
	return count;
}

static ssize_t input_store(struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    for (i=0; i<FOCUS_PROC_MAX; i++){
	 	memset(input_mgr_proc[i].procname, 0, 64);
		input_mgr_proc[i].enable = 0;
		input_mgr_proc[i].pid = -1;
 	}
 
	for (i=0; i<FOCUS_PROC_MAX; ){
	    ...
	    p = strstr(pbuf, ",");
	 	if(p == NULL){
			...
			n = 0;
			if(p - pbuf < 64){
		    	memcpy(data, pbuf, p - pbuf);
			}else{
				memcpy(data, pbuf, 63);
			}
			
			while(isspace(data[n]) && n < p - pbuf){
				n++;
			}         
			m = p - pbuf - n;		
			ptr = &data[n];
			...
			if(n >= m){
				ret = kstrtouint(&ptr[0], 0, &(input_mgr_proc[i].pid));
                if(ret<0){
    				printk("kstrtouint error:%d.\n",ret);
                }
            }else{				
	            memcpy(input_mgr_proc[i].procname, &ptr[0], m);
				printk("input procname=%s \n",input_mgr_proc[i].procname);
			}
			
			input_mgr_proc[i].enable = 1;
			pbuf = p + 1;
			i++;
		}
...
//static struct kobj_attribute input_foucs_show =__ATTR(read, 0660, input_show, NULL);

//static struct kobj_attribute input_foucs_store =__ATTR(write, 0660, NULL, input_store);

static DEVICE_ATTR(read, 0660,    input_show, NULL);
static DEVICE_ATTR(write, 0660,    NULL, input_store);

static struct attribute *input_foucs_attr[] = {
    &dev_attr_read.attr,
    &dev_attr_write.attr,
    NULL,
};
...
static struct attribute_group my_attr_group = {
    .attrs = input_foucs_attr,
};
    
static int sysfs_status = 0 ;
static struct kobject *soc_kobj = NULL;
			 ...
			 const struct input_device_id *id)
{
	...
	//add by fk
	if(sysfs_status == 0){
		 soc_kobj = kobject_create_and_add("inputFocus", NULL);
		 if (!soc_kobj)
		  goto err_board_obj;
